...
def test(packetName, jsonString, results):
    try:
        exiProcessor = EXIProcessor(ProtocolEnum.DIN)
...
def main():
    ...
    headerJson = json.load(open(projectRoot + "/test/DIN/DIN_Header.json"))

    for filePath in sorted(glob(projectRoot + "/test/DIN/*.json")):
        if "Header" in os.path.basename(filePath):
            continue

        packetName = os.path.basename(filePath).split(".")[0]
        jsonString = json.load(open(filePath))
        jsonString.update(headerJson)
