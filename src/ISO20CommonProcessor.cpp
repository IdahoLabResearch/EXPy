...
#include <common.hpp>
#include "ISO20CommonProcessor.hpp"
...
// Marshalers — generated by tools/codegen from iso20_CommonMessages_Datatypes.h.
#include "generated/ISO20Common_marshalers.generated.cpp"
...
extern "C" {
    ...
    encoded_data* encodeFromPythonDict(const char* inString) {
        try {
            ...
            int rc = encode_iso20_exiDocument(&outEXI, &inDoc);
    ...
    decoded_data* decode_fragment_FromPythonBytes(const uint8_t* data, size_t size) {
        try {
            iso20_exiFragment outDoc;
            exi_bitstream_t inEXI;

            exi_bitstream_init(&inEXI, const_cast<uint8_t*>(data), size, 0, nullptr);
            int rc = decode_iso20_exiFragment(&inEXI, &outDoc);
    ...
    encoded_data* encode_fragment_FromPythonDict(const char* inString) {
        try {
            ...
            exi_bitstream_init(&outEXI, stream, 256, pos1, nullptr);
            int rc = encode_iso20_exiFragment(&outEXI, &inDoc);
    ...
    decoded_data* decode_xmldsig_FromPythonBytes(const uint8_t* data, size_t size) {
        try {
            iso20_xmldsigFragment outDoc;
            exi_bitstream_t inEXI;

            exi_bitstream_init(&inEXI, const_cast<uint8_t*>(data), size, 0, nullptr);
            int rc = decode_iso20_xmldsigFragment(&inEXI, &outDoc);
    ...
    encoded_data* encode_xmldsig_FromPythonDict(const char* inString) {
        try {
            ...
            exi_bitstream_init(&outEXI, stream, 256, pos1, nullptr);
            int rc = encode_iso20_xmldsigFragment(&outEXI, &inDoc);
