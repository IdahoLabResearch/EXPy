"""Integration roundtrip over generator-produced ISO20_COMMON Fragment payloads.

Enumerates the members of the `iso20_exiFragment` union and roundtrips each
...
import expy.v2gjson.iso20_common as v2gjson_iso20_common  # noqa: E402
...
ISO20_COMMON_FRAGMENT_OVERRIDES: dict = {}


def _discover_fragment_members():
    """Pull (struct-type, element-name) for each member of
    `iso20_exiFragment`'s inner union, in declaration order.
    """
    spec = _SPECS["iso20_exiFragment"]
...
def _scenarios():
    for type_name, element in _FRAGMENT_TYPES:
        yield from emit_fragment_scenarios(
            ...
            specs=_SPECS,
            enum_names=_ENUMS,
            overrides=ISO20_COMMON_FRAGMENT_OVERRIDES,
...
def test_iso20_common_generated_fragment_roundtrip(scenario):
    _scenario_id, payload = scenario
    assert_roundtrip("ISO20_COMMON", payload, kind="fragment")
