"""Integration roundtrip over generator-produced ISO20_AC Fragment payloads.

Enumerates the members of the `iso20_ac_exiFragment` union and roundtrips each
...
import V2Gjson.iso20_ac as v2gjson_iso20_ac  # noqa: E402
...
ISO20_AC_FRAGMENT_OVERRIDES: dict = {}


def _discover_fragment_members():
    spec = _SPECS["iso20_ac_exiFragment"]
...
def _scenarios():
    for type_name, element in _FRAGMENT_TYPES:
        yield from emit_fragment_scenarios(
            ...
            specs=_SPECS,
            enum_names=_ENUMS,
            overrides=ISO20_AC_FRAGMENT_OVERRIDES,
...
def test_iso20_ac_generated_fragment_roundtrip(scenario):
    _scenario_id, payload = scenario
    assert_roundtrip("ISO20_AC", payload, kind="fragment")
